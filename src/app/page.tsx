  ...
  return (
    <>
      ...
      <section className="section container projects-section">
        ...
        <div className="project-grid">
          {projects.map((project, index) => (
            <article className={`project-card ${index % 2 === 0 ? "project-accent" : ""}`} key={project.title}>
      ...
      <section className="section container">
        <div className="offer"><h2>What We Offer</h2><div className="offer-grid">{equipment.map((item) => (
          <article className="offer-card" key={item.title}><div><h3>{item.title}</h3><p>{item.description}</p></div><div className="offer-image"><Image src={imagePath(item.image)} alt={`Hình minh họa: ${item.title}`} fill sizes="(max-width: 760px) 40vw, 16vw" /></div></article>
