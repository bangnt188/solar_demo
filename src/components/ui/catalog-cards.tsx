...
export function ProjectCard({ project, accent = false }: { project: Project; accent?: boolean }) {
  return (
    <article className={`project-card ${accent ? "project-accent" : ""}`} data-motion="up">
...
export function EquipmentCard({ item }: { item: Equipment }) {
  return (
    <article className="offer-card" data-motion="fade"><div><h3>{item.title}</h3><p>{item.description}</p></div><div className="offer-image"><Image src={imagePath(item.image)} alt={`Hình minh họa: ${item.title}`} fill sizes="(max-width: 760px) 40vw, 16vw" /></div></article>
  );
}
